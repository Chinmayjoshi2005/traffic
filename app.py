...
# -------------------
# Logging thread (safe once-per-minute)
# -------------------
            ...
            with lock:
                for lane_zero_index, count in vehicle_counts.items():
                    ...
                    try:
                        ...
                        db.session.rollback()  # ignore duplicate insert
...
def api_logs():
    ...
    return jsonify([
        {"lane": log.lane, "vehicles": log.vehicles, "timestamp": log.timestamp.strftime("%Y-%m-%d %H:%M:%S")}
        for log in logs
    ])
